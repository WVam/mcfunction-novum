const versions = ["1.18-1.18.1","1.17-1.17.1","1.16.5"];
...
function setArrayList(jsonName, version) {
	let list = require("./" + versions[versions.length - 1] + "/id/" + jsonName).slice();

	let versionIndex = versions.indexOf(version);
	let i = versions.length - 2;

	while(i >= versionIndex && i >= 0) {
...
function setObjectList(jsonName, version) {
	let object = JSON.parse(JSON.stringify(require("./" + versions[versions.length - 1] + "/id/" + jsonName)));
